...
function canSyncWithoutAuth(){
  // Fallback temporal de pruebas: solo Rancho1.
  return syncState.authDisabled && getRanchoId() === 'Rancho1';
}
...
function initFirebaseAuthUi(){
  ...
  const refreshUi = (user) => {
    ...
    if (syncState.authDisabled) {
      const fallback = canSyncWithoutAuth()
        ? ' Mientras tanto puedes guardar snapshots en Rancho1 (modo prueba temporal).'
        : '';
      setMessage((syncState.authDisabledReason || getAuthSetupInstructions()) + fallback, true);
    }
  };
...
async function pushSnapshot(key, payload){
  ...
  const allowNoAuth = canSyncWithoutAuth();
  if (syncState.authDisabled && !allowNoAuth) return;
  if (!auth.currentUser && !allowNoAuth) return;
...
function getStatus(){
  return {
    ...
    authlessTestingSync: canSyncWithoutAuth(),
    ...syncState
  };
}
